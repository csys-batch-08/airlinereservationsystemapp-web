		...
		<tbody>
			<c:forEach items="${CancelList}" var="cancel" varStatus="status">
				<tr>
					...
					<fmt:parseDate value="${cancel.getArrival_date()}"
						pattern="yyyy-MM-dd" var="macthDate" type="date" />
					<td><fmt:formatDate value="${macthDate}" pattern="dd-MM-yyyy" />
					</td>
					<td>${cancel.getTicketNo()}</td>
					<td class="success">${cancel.getSeatno()}</td>
					<td class="danger">${cancel.getStatus()}</td>
					<fmt:parseDate value="${cancel.getBookingdate()}"
						pattern="yyyy-MM-dd" var="bookingdate" type="date" />
					<td><fmt:formatDate value="${bookingdate}"
					...
					<c:choose>
						<c:when test="${cureendate<cancel.getArrival_date()}">
							<c:choose>
								...
								<c:otherwise>
									<td><a
										href="flightcancellation.jsp?seatno=${cancel.getSeatno()}&Flightid=${cancel.getFlight_id()}&Departuredate=${cancel.getArrival_date()}
