...
%> --%>
	...
	<c:if test="${flights!= null}">
		...
		<a href="Sourcedestination"
			style="position: relative; left: 339px; top: 70px; width: 140px; height: 29px; font-style: italic;"
			...
			<tbody>
				...
				<c:forEach items="${Flight_List}" var="plane" varStatus="status">
					...
					<tr>
						...
						<fmt:parseDate value="${plane.getDeparture_Date()}"
							pattern="yyyy-MM-dd" var="departured" type="date" />

						<td><fmt:formatDate value="${departured}"
								pattern="dd-MM-yyyy" /></td>
						<fmt:parseDate value="${plane.getArrival_Date()}"
							pattern="yyyy-MM-dd" var="arrivied" type="date" />

						<td><fmt:formatDate value="${arrivied}" pattern="dd-MM-yyyy" />
						</td>
						...
						<c:if test="${loggedinuser != null}">
							...
							<c:if
								test="${plane.getEconomy_class()==0 && plane.getPremium_Economy_class()== 0 && plane.getBussiness_class()==0 }">
							...
							<c:if
								...
								<td><a
									href="Addpassenger?flightId=${plane.getFlightId()}&source=${plane.getSource()}&destination=${plane.getDestination()}
